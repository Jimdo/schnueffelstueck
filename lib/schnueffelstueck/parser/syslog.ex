defmodule Schnueffelstueck.Parser.Syslog do
  ...
  defp generate_metrics(time, [method, _path, status, bytes, cache_hit, num_hits, lastuse, latency], source) do
    [
      ...
      %Metric{name: :bytes, value: parse_int(bytes), source: source, measure_time: time },
      %Metric{name: :num_hits, value: parse_int(num_hits), source: source, measure_time: time },
      %Metric{name: :lastuse, value: parse_float(lastuse), source: source, measure_time: time },
      %Metric{name: :latency, value: parse_float(latency), source: source, measure_time: time }
    ]
  end
